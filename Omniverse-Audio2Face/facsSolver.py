...
class FacsSolver:
    def __init__(self, neutral_mat, delta_mat):
        ...
        # auto-set the blendshape solve regularization scale
        self.compute_scale_factors()

        self.compute_A_mat()

    def compute_scale_factors(self):
        neutral_mat_3 = self.neutral_mat.reshape(-1, 3)
        target_bb_size = np.linalg.norm(np.max(neutral_mat_3, axis=0) - np.min(neutral_mat_3, axis=0))
        scale_factor = (target_bb_size / self.template_bb_size) ** 2
        self.weightRegulCoeff_scale *= scale_factor
        self.prevRegulCoeff_scale *= scale_factor
        self.sparseRegulCoeff_scale *= scale_factor
        self.symmetryRegulCoeff_scale *= scale_factor
        log_info(f"The scale factor is adjusted for the target face, scale factor: {scale_factor}")
